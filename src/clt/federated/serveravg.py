...
import time
import numpy as np
from clientavg import clientAvg
...
class FedAvg(object):
    def __init__(self, cfg, times):
        ...
        self.folder = f"{self.start_str}/{self.goal}/"
    ...
    def train(self):
        for i in range(self.global_rounds+1):
            ...
            if self.adapft:
                self.receive_adapter_head()
                self.aggregate_adapter_head()
            else:
                self.receive_models()
    ...
    def evaluate_and_save_model(self, rnd, acc=None, loss=None):
        ...
        self.save_model(rnd=rnd, save_dir=os.path.join(self.model_dir, "models"))
    ...
    def send_adapter_head(self):
        assert len(self.clients) > 0

        for client in self.clients:
            start_time = time.time()
            adapter_state = self.global_model.adapter.state_dict()
            head_state = self.global_model.head.state_dict()
            client.set_adapter_head(adapter_state, head_state)
            client.send_time_cost['num_rounds'] += 1
            client.send_time_cost['total_cost'] += 2 * (time.time() - start_time)
    ...
    def receive_adapter_head(self):
        ...
        for client in active_clients:
            ...
            if client_time_cost <= self.time_threthold:
                ...
                self.uploaded_ids.append(client.id)
                self.uploaded_weights.append(client.train_samples)
                adapter_state, head_state = client.get_adapter_head()
                self.uploaded_adapters.append(adapter_state)
                self.uploaded_heads.append(head_state)
    ...
    def aggregate_adapter_head(self):
        ...
        # Head aggregation
        new_head = copy.deepcopy(self.global_model.head.state_dict())
        for key in new_head:
            new_head[key] = sum(
                w * client_head[key] for w, client_head in zip(self.uploaded_weights, self.uploaded_heads)
            )

        # Apply
        self.global_model.adapter.load_state_dict(new_adapter)
        self.global_model.head.load_state_dict(new_head)
    ...
    def save_model(self, rnd, save_dir=None):
        ...
        file_name = f"model_{rnd}.pt"
        save_path = os.path.join(save_dir, file_name)

        if self.adapft:
            state_dict = {
                'adapter': self.global_model.adapter.state_dict(),
                'head': self.global_model.head.state_dict()
            }
        else:
