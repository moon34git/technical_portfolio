...
class clientRep(object):
    ...
    def train(self):
        ...
        for _ in range(self.local_epochs):
            for x, y in trainloader:
                ...
                self.scaler.scale(loss).backward()
                self.scaler.unscale_(self.optimizer)
                torch.nn.utils.clip_grad_norm_(self.model.adapter.parameters(), 1.0)
                self.scaler.step(self.optimizer)
                self.scaler.update()
    ...
    def set_adapter(self, adapter_state):
        self.model.adapter.load_state_dict(adapter_state)

    def get_adapter(self):
        return copy.deepcopy(self.model.adapter.state_dict())
